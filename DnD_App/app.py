...
# --- NEU: Wetter & Zeit Tracker ---
class CampaignState(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    day = db.Column(db.Integer, default=1)
    month = db.Column(db.String(50), default='Flamerule')
    year = db.Column(db.Integer, default=1489)
    weather = db.Column(db.String(50), default='sunny') # sunny, rain, snow, storm
...
with app.app_context():
    ...
    if not CampaignState.query.first():
        db.session.add(CampaignState(day=1, month='Flamerule', year=1489, weather='sunny'))
        db.session.commit()
